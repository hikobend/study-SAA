...
S3に保存。頻繁にアクセスするデータサブネットのコピーをローカルに保存。頻繁にアクセスするデータへのアクセスを低レイテンシーを保つ

・ボリュームStorageGateway

プライマリはオンプレに。データセット全体への低レイテンシーが必要な場合、全てのデータを最初にローーカルに保存。

・SystemMananger

AWSサービスのオペレーションデータを表示し、AWSリソース間でオペレーションタスクの自動化できる。運用データの可視化、一元化可能。
...
AWS KMSがキーの生成・管理・保管を行う。料金が発生。コンプライアンスの関係など、本格的な暗号化キーを管理。キーの利用状況をCloudTrailで追跡可能。アクセスを制御。
